...
var addPlayerClicked=()=>{
    ...
    detailTeam = JSON.parse(localStorage.getItem("teamArray"));
    detailPlayer = JSON.parse(localStorage.getItem("playerArray"));
...
for(var i=0;i<detailTeam.length;i++){
    suggestArr.push(detailTeam[i].sName)
...
inputBox.onkeyup = (e)=>{
    ...
    if(userData){
        ...
        emptyArray = suggestArr.filter((data)=>{
...
<img src="${detailTeam[i].teamIcon}" class="mainimage" alt=""/> 
...
function makethisinclick(res){
    var clickedCard=detailTeam[res].sName
